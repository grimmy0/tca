...
- UI auth: `/ui/login` (GET/POST), `/ui/logout` (POST) are unauthenticated. All other `/ui` routes accept bearer token or HMAC-signed session cookie (`Path=/ui`, 24h expiry, per-startup signing key).
