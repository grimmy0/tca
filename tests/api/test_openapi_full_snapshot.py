"""OpenAPI contract snapshot tests for all Phase 1 API endpoints."""
...
SNAPSHOT_PATH = (
    PROJECT_ROOT / "tests" / "api" / "snapshots" / "phase1_openapi_snapshot.json"
)
UPDATE_ENV_VAR = "TCA_UPDATE_OPENAPI_FULL_SNAPSHOT"
...
REQUIRED_SCHEMA_FIELDS = {
    "TelegramAuthStartRequest": {"api_id", "api_hash", "phone_number"},
    "TelegramAuthStartResponse": {"session_id"},
    "TelegramAuthVerifyCodeRequest": {"session_id", "api_id", "api_hash", "code"},
    "TelegramAuthVerifyCodeResponse": {"session_id", "status"},
    ...
    "TelegramAuthVerifyPasswordResponse": {"session_id", "status"},
    "ChannelCreateRequest": {"account_id", "telegram_channel_id", "name", "username"},
    "ChannelPatchRequest": {
        "name",
        "username",
        "is_enabled",
    ...
    "PollNowResponse": {"channel_id", "correlation_id"},
}


def test_openapi_snapshot_includes_all_phase1_endpoints_and_fields(
    tmp_path: Path,
    monkeypatch: object,
) -> None:
    """Ensure snapshot includes all targeted Phase 1 paths and payload fields."""
    openapi = _load_openapi(tmp_path=tmp_path, monkeypatch=monkeypatch)
    snapshot = _build_phase1_snapshot(openapi=openapi)
...
def test_openapi_snapshot_matches_committed_phase1_contract(
    tmp_path: Path,
    monkeypatch: object,
) -> None:
    """Ensure contract drift fails until reviewed Phase 1 snapshot updates."""
    openapi = _load_openapi(tmp_path=tmp_path, monkeypatch=monkeypatch)
    actual = _build_phase1_snapshot(openapi=openapi)
    ...
    if actual != expected:
        ...
        message = (
            "OpenAPI contract drift detected for Phase 1 endpoints.\n"
            f"Run `{UPDATE_ENV_VAR}=1 uv run pytest -q "
            "tests/api/test_openapi_full_snapshot.py` after reviewing the contract "
            f"diff.\n{diff}"
        )
...
def _load_openapi(*, tmp_path: Path, monkeypatch: object) -> dict[str, object]:
    ...
    db_path = tmp_path / "openapi-phase1.sqlite3"
...
def _build_phase1_snapshot(*, openapi: dict[str, object]) -> dict[str, object]:
    """Build deterministic OpenAPI subset for Phase 1 endpoint contracts."""
